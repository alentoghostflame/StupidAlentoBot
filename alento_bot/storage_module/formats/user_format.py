...
class UserClass(SaveLoadConfig, path="you_shouldnt_see_this_user.yaml"):
    def __init__(self, config: ConfigData, user_id: int):
        super().__init__()
        self._data_path = f"{config.data_folder_path}/users/{user_id}/{self._name}"
    ...
    def __init_subclass__(cls, name: str = "default_cache_name", **kwargs):
        super().__init_subclass__(path="you_shouldnt_see_this_user.yaml")
        cls._name: str = name
