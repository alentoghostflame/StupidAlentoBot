...
from OLD.steam_announcement_module import text
...
async def set_channel(server_data: DiskServerData, context: commands.Context, arg=None):
    ...
    channel = context.guild.get_channel(OLD.universal_module.utils.get_numbers(arg))
...
async def add_game(server_data: DiskServerData, context: commands.Context, arg=None):
    steam_id = OLD.universal_module.utils.get_numbers(arg)
    if not arg:
