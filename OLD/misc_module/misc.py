from OLD.misc_module.callout_delete import callout_delete, callout_delete_admin
from OLD.misc_module.welcome import welcome, welcome_admin
from OLD.storage_module.disk_storage import DiskStorage
from OLD.storage_module.ram_storage import RAMStorage
from OLD.misc_module.userinfo import userinfo
from OLD.misc_module.status import bot_status
...
class MiscCog(commands.Cog, name="Misc Module"):
    ...
    async def administrator_permission_error(self, context, error):
        if isinstance(error, commands.MissingPermissions):
            await context.send(OLD.universal_module.text.PERMISSION_MISSING_ADMINISTRATOR)
