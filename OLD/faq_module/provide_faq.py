from OLD.storage_module.server_data import FAQPhraseData
    ...
    for keyword in get_keywords(message_content):
        if keyword in faq_phrases and keyword not in found_keys:
            ...
            if just_keywords(phrase_data.statement):
                ...
                embed.add_field(name=keyword, value=phrase_data.statement + OLD.universal_module.text.ZERO_WIDTH_SPACE)
...
def migrate_to_class(faq_phrases: typing.Dict[str, FAQPhraseData], keyword: str):
    ...
    if OLD.universal_module.utils.is_image_url(phrase_data.statement):
